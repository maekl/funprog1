 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of the empty list") {
    assert(sum(List()) === 0)
  }

  test("sum of list with zeros only") {
  ...
  test("sum of the n first positive ints is n * (n + 1) / 2") {
    (1 to 1000).foreach(n => assert(sum(List.range(1, n + 1)) === n * (n + 1) / 2))
  }

  test("sum of lists of 1s equals the length") {
    (1 to 1000).foreach { n =>

      val ones = List.fill(n)(1)

      assert(sum(ones) === ones.length)
    }
  }
}
